---
id: 0019
title: Command-log persistence for engine-api-http
status: Deferred
phase: P8a
opened: 2026-05-27
depends-on: [0018]
...
writes:
  ...
  forbid:
    - Engine.Cli/**
notes: Re-filed from branch claude/happy-booth-1cef3f, where it carried the number 0012. See register entry R-0012.
---
# TASK-0019 — Command-log persistence for `engine-api-http` (P8a)

## Status

Deferred. Re-filed 2026-09-20 by TASK-0018.

This task came from branch `claude/happy-booth-1cef3f`, which nobody merged. It carried the number
...
ADR-0015 (Accepted 2026-05-27) pre-decides every load-bearing call:
...
- `Engine.Core/Persistence/FileCommandLog.cs` — new, the V1.x file-backed implementation:
  - Constructor: `FileCommandLog(string path)`.
    - Opens `FileStream(path, FileMode.OpenOrCreate, FileAccess.ReadWrite, FileShare.None)` — `FileShare.None` is the exclusive lock per ADR-0015 §7.
...
- `Engine.Core/Persistence/CommandLogRecord.cs` — new. DTOs for the two record kinds; JSON shapes per ADR-0015 §3. Uses `System.Text.Json` with camelCase. Includes a `RecordType` discriminator (`"log-header"` / `"command-applied"`) so the line parser dispatches.
...
**None.** The CLI stays ephemeral per ADR-0015 §5. It does not read `ENGINE_LOG_PATH`, does not open the file, does not replay. CI guard test asserts this (§5 below).
...
- **Snapshots / compaction.** ADR-0015 §6 defers; revisit when log size warrants.
- **Embedded-CLI persistence.** ADR-0015 §5 + §Open challenges. Out of V1.x.
...
- **Command-version migration / cross-version log upgrade.** ADR-0015 §8 + §Open challenges. Operators preserve handlers.
...
- **ADR-0015 — Command-log persistence for `engine-api-http`** (Accepted this set).
...
- `Engine.Tests/Http/EngineHostStartupTests.cs`
- `Engine.Tests/Cli/CliPersistenceScopeTests.cs`
- `tasks/TASK-0019-command-log-persistence.md` (this file)
...
- `tasks/TASK-0019-command-log-persistence.md` — Status flip in close commit.

**Do not touch:**
- `Engine.Contracts/**` — no public-shape change. ADR-0015 §Consequences pins this.
...
- **No `Engine.Contracts` changes.** If a need arises, stop and flag — those are gated by ADR. ADR-0015 deliberately keeps the abstraction in `Engine.Core`.
