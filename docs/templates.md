...
| Field | Rule |
|---|---|
| `status` | Use one value: `Proposed`, `Accepted`, `Superseded-by-NNNN`, `Withdrawn` or `Rejected`. `Withdrawn` means that the author changed the proposal. `Rejected` means that the project examined the proposal and refused it. |
...
| `enforced-by` | Give a test name, an analyzer rule, or the text `UNENFORCED (accepted risk)`. The gate fails if the quantity of unenforced ADRs increases. |
