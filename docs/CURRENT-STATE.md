...
## v0.12 — Engine hosting factory (refactor; TASK-0013)

Shared engine-wiring factory in `Engine.Core/Hosting/EngineHosting.cs` eliminates the duplicate bring-up sequences that previously lived in `Engine.Cli/Cli.BuildEngine` and `Engine.Api.Http/EngineHost`. `CreateDefault()` returns an `EngineKit` record `(Document, CommandRegistry, QueryRegistry, InMemoryEventSink, InProcessMeshBackend)`; callers compose `CommandBus`/`QueryBus` themselves so the HTTP host can wrap the sink in `BroadcastingEventSink` (TASK-0010) while the CLI uses the sink directly. `RegisterDefaultCommands` and `RegisterDefaultQueries` are exposed separately so a host with custom registries can layer V1.x defaults. `EngineHost.cs` drops `using Engine.Core.Commands|Geometry|Queries`. `Cli.BuildEngine` shrinks from 11 lines to 4. Pure refactor; no behavioural change to commands, queries, events, schemas, or the wire surface. No `Engine.Contracts/**` change. No new diagnostic codes. No ADR — same-namespace housekeeping. 12 new factory tests; existing 105 unchanged. `dotnet build` + `dotnet test` green (117 total). From this point forward, TASK numbers no longer track CURRENT-STATE version numbers (TASK-0013 ships as v0.12; the pre-drafted TASK-0012 will ship later as v0.13).
